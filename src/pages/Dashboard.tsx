...
import { BookOpen, CheckCircle, Clock, Play, Flame, Loader2 } from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const handleUpdate = (event: any) => {
      setTotalSeconds(event.detail);
    };

    window.addEventListener('studyTimeUpdated', handleUpdate);
    return () => window.removeEventListener('studyTimeUpdated', handleUpdate);
  }, []);
